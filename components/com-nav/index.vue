<template>
    <div>
        ...
        <ul class="left-nav" v-if="list.length > 3 && count > 0">
            ...
            <li @click="handleScroll(item, item.index)" class="left-nav-title"
                :class="{ active: checkIndex === item.index }" v-for="item in leftNavList" :key="item.index">
                {{ item.title }}
            </li>
            <li class="left-nav-title add-link" @click="showAddLink = true;">
                提交网址
            </li>
        </ul>
        <BackTop></BackTop>
        <ComAddlink :show.sync="showAddLink"></ComAddlink>
    </div>
</template>
    ...
    .row {
        ...
        .col {
            ...
            .card-link {
                ...
                &:hover {
                    -moz-box-shadow: 4px 4px 10px var(--ion-color-step-50, #e9edf1);
